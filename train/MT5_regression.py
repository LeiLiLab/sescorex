...
class Regression_XLM_Roberta(nn.Module):
    def __init__(self, model_addr):
        super().__init__()
        self.xlm = MT5EncoderModel.from_pretrained(model_addr,cache_dir='/mnt/data5/zihanma')
...
def main(lang_dir, gradient_accumulation_steps, batch_size, emb_type, num_epoch, eval_step, \
    ...
    dist.init_process_group(backend="nccl", timeout=datetime.timedelta(seconds=4500))
...
#        })
    ...
    tokenizer = AutoTokenizer.from_pretrained("google/rembert",cache_dir='/mnt/data5/zihanma')
    if load_file_enable:
        model = torch.load(model_addr, map_location="cpu").to(exp_config.device_id)
    else:
        if emb_type == 'last_layer':
            model = Regression_XLM_Roberta("google/mt5-xl").to(exp_config.device_id)
        else:
    ...
    if gradual_unfrozen:
        model.freeze_xlm() # freeze the xlm parameters
    num_epoch = 10
    with torch.autograd.set_detect_anomaly(True):
        for epoch in range(num_epoch):
            ...
            with tqdm(total=len(train_dataloader), desc="Outer Loop", position=0) as pbar:
                for step, train_batch in enumerate(train_dataloader):
                    ...
                    if (step % (eval_step * gradient_accumulation_steps * 5) == 0):
                        cur_best_cor = float('-inf')
                    # evaluate at every eval_step and also at the end of epoch (includes the beginning loss)
                    if (((step+epoch) % (eval_step * gradient_accumulation_steps) == 0) or (step == len(train_dataloader) - 1)) and rank == 0:
                        ...
                        #    mt_scores_dict = baselines_cl_eval_en_ja(out_lines, ref_lines, emb_type, model, 150, tokenizer)
                        #    step_seg_cor, save_seg_dict = store_corr_eval_en_ja(evs, mt_scores_dict, gt_score_dict)
                        #    wandb_temp_dict.update(save_seg_dict)
                        #else:
                        #    mt_scores_dict = baselines_cl_eval(mt_outs_dict, refs, emb_type, model, 150, tokenizer)
                        ...
                        mt_scores_dict = baselines_cl_eval(mt_outs_dict, refs, emb_type, model, 128, tokenizer)
                        step_seg_cor, save_seg_dict = store_corr_eval(evs, mt_scores_dict, 'seg', wmt, lang)
                        _, save_sys_dict = store_corr_eval(evs, mt_scores_dict, 'sys', wmt, lang)
                        print("Testing Duration: ", time.time()-start_eval_time)

                        print(f"Current step_seg_cor: {step_seg_cor}")
                        if step_seg_cor > cur_best_cor:
                            cur_best_cor=step_seg_cor
                            torch.save(model.module, f'{save_dir_name}/best_rem_en-de_{epoch}_{step}.ckpt')
