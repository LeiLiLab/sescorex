...
class Regression_XLM_Roberta(nn.Module):
    def __init__(self, model_addr):
        super().__init__()
        self.xlm = AutoModel.from_pretrained(model_addr,cache_dir='/mnt/data5/zihanma')
...
class loss_funct():
    def __init__(self):
        ...
        #self.loss_fct2 = nn.MSELoss()

    # If src_based, ref will be used as pos samples (pos:0, batchNeg: 6)->max_diff=6. If ref_based, max_diff=5
    def loss_compute(self, batch, model, emb_type, batch_size):
        score = model(batch, emb_type)
        print(f"score: {score}")
        print(f"score 1: {score[:batch_size]}")
        print(f"score 2: {score[batch_size:]}")
        loss1 = self.loss_fct(score[:batch_size], batch['score'][:batch_size].unsqueeze(1).to(exp_config.device_id))
        print("loss1: ", loss1)
        loss2 = self.loss_fct(score[batch_size:], batch['score'][batch_size:].unsqueeze(1).to(exp_config.device_id))
        print("loss2: ", loss2)
        return loss1, loss2
        #return loss1
...
def store_regression_loss(model, loss_manager, train_batch, emb_type, batch_size):
    with torch.no_grad():
        save_dict = {}
        train_loss1, train_loss2 = loss_manager.loss_compute(train_batch, model, emb_type, batch_size)
        save_dict['train_loss1'] = train_loss1.item()
        save_dict['train_loss2'] = train_loss2.item()
    return save_dict
...
@click.option('-data_file1', type=str, help="fixed_wmt_news_cl_10_en_zh_rand_True_5.txt", default="fixed_wmt_news_cl_10_en_zh_rand_True_5.txt")
...
def main(lang_dir, gradient_accumulation_steps, batch_size, emb_type, num_epoch, eval_step, \
            num_warmup_steps, save_dir_name, enable_loss_eval, data_file, data_file1, gradual_unfrozen, lr, model_base, hidden_size, \
            model_addr, load_file_enable, test_ted_enable, alpha):
    # load in eval data
    #print(f"batch_size: {batch_size}")
    parent_dir = '/home/zihanma/Sescore2_main'
    num_epoch = 2
    sys.path.append(parent_dir)
    exp_config.lr = lr
    exp_config.hidden_size = hidden_size
    exp_config.hidden_size_FNN=[hidden_size*2, hidden_size]
    wmt, lang = 'wmt22', lang_dir.replace('_', '-')
    ...
    tokenizer = AutoTokenizer.from_pretrained("xlm-roberta-large",cache_dir='/mnt/data5/zihanma')
    if load_file_enable:
        model = torch.load(model_addr, map_location="cpu").to(exp_config.device_id)
    else:
        if emb_type == 'last_layer':
            model = Regression_XLM_Roberta("xlm-roberta-large").to(exp_config.device_id)
        else:
    ...
    for line in data_lines:
        line_ls = line.split('<ENDSENTENCE>')
    ...
    data_lines1 = open(data_file1, 'r').readlines()
    ref_ls1, mt_ls1, score_ls1 = [], [], []
    data_lines1 = [ele[:-1] for ele in data_lines1]
    for line in data_lines1:
        line_ls = line.split('<ENDSENTENCE>')
        ref, mt, score = line_ls[0], line_ls[1], line_ls[2]
        ref_ls1 += [ref]
        mt_ls1 += [mt]
        score_ls1 += [float(score)]
    triplets_score_train_dict1 = {'pivot': ref_ls, 'mt': mt_ls, 'score': score_ls}

    #ratio_1 = len(ref_ls) / (len(ref_ls) + len(ref_ls1))
    #ratio_2 = len(ref_ls1) / (len(ref_ls) + len(ref_ls1))
    #total_steps_per_epoch = batch_size
    #batches_per_iteration_2 = int(ratio_2 * total_steps_per_epoch)

    #print(f"Batch size 1: {batches_per_iteration_1}")
    #print(f"Batch size 2: {batches_per_iteration_2}")

    train_dataloader_1 = preprocess_data(triplets_score_train_dict, tokenizer, exp_config.max_length, batch_size, shuffle=True, sampler=True, mode='train')
    train_dataloader_2 = preprocess_data(triplets_score_train_dict1, tokenizer, exp_config.max_length, batch_size, shuffle=True, sampler=True, mode='train')

    model = DDP(model, device_ids=[exp_config.device_id], find_unused_parameters=True)
    model.train()
    max_train_steps = math.ceil((len(train_dataloader_1)) / gradient_accumulation_steps) * num_epoch
    ...
    with torch.autograd.set_detect_anomaly(True):
        for epoch in range(num_epoch):
            ...
            train_dataloader_1.sampler.set_epoch(epoch) # set the sampler at each epoch
            train_dataloader_2.sampler.set_epoch(epoch) # set the sampler at each epoch
            train_dataloader_2_repeated = cycle(train_dataloader_2)

            with tqdm(total=len(train_dataloader_1)) as pbar:
                for step, (train_batch_1, train_batch_2) in enumerate(zip(train_dataloader_1, train_dataloader_2_repeated)):

                    combined_batch = {}
                    for key in train_batch_1.keys():
                        combined_batch[key] = torch.cat((train_batch_1[key], train_batch_2[key]), dim=0)
                    ...
                    if (step % (eval_step * gradient_accumulation_steps * 5) == 0):
                        cur_best_cor = float('-inf')
                    if (((step+epoch+1) % (eval_step * gradient_accumulation_steps) == 0) or (step == len(train_dataloader_1) - 1)) and rank == 0:
                    ...
                    #        mt_scores_dict = baselines_cl_eval_en_ja(out_lines, ref_lines, emb_type, model, 60, tokenizer)
                    #        step_seg_cor, save_seg_dict = store_corr_eval_en_ja(evs, mt_scores_dict, gt_score_dict)
                    #        wandb_temp_dict.update(save_seg_dict)
                    #    else:
                    #        mt_scores_dict = baselines_cl_eval(mt_outs_dict, refs, emb_type, model, 60, tokenizer)
                    ...
                    #        wandb_temp_dict.update(save_sys_dict)
                        ...
                        if enable_loss_eval:
                            save_loss_dict = store_regression_loss(model, loss_manager, combined_batch, emb_type, batch_size)
                            wandb_temp_dict.update(save_loss_dict)

                        mt_scores_dict = baselines_cl_eval(mt_outs_dict, refs, emb_type, model, 100, tokenizer)
                        step_seg_cor, save_seg_dict = store_corr_eval(evs, mt_scores_dict, 'seg', wmt, lang)
                        _, save_sys_dict = store_corr_eval(evs, mt_scores_dict, 'sys', wmt, lang)
                        wandb_temp_dict.update(save_seg_dict)
                        wandb_temp_dict.update(save_sys_dict)
                        wandb.log(wandb_temp_dict)
                        print("Training loss: ", save_loss_dict)
                        print("Testing Duration: ", time.time()-start_eval_time)

                        # save at the best epoch step
                        #print(f"step_seg_cor: {step_seg_cor}")
                        #print(f"{save_dir_name}/best_{step}.ckpt")
                        if step_seg_cor > cur_best_cor:
                            cur_best_cor=step_seg_cor
                            torch.save(model.module, f'{save_dir_name}/best_{lang}_{epoch}_{step}.ckpt')
                        print("step_seg_cor: ", step_seg_cor)
                        print(f"Saved best model at current epoch {epoch} step {step}!")
                        model.train()
                        torch.cuda.empty_cache()

                    if gradual_unfrozen and (global_step == int(len(train_dataloader_1)*alpha)):
                        # unfreeze the model paramters after the first evaluation step
                        model.module.unfreeze_xlm()
                    train_loss1, train_loss2 = loss_manager.loss_compute(combined_batch, model, emb_type, batch_size)
                    #train_loss2 = loss_manager.loss_compute(train_batch_2, model, emb_type)
                    #print(train_loss1)
                    #print(train_loss1.item())
                    #print(train_loss2)
                    ...
                    train_loss = train_loss.to(exp_config.device_id)
                    #train_loss2 = train_loss2 / gradient_accumulation_steps
                    train_loss.backward()
                    #train_loss2.backward()
                    if step % gradient_accumulation_steps == 0 or step == len(train_dataloader_1) - 1:
                        optimizer.step()
                        lr_scheduler.step()
