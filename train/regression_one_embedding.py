...
class Regression_XLM_Roberta(nn.Module):
    ...
    def forward(self, batch, emb_type):
        sentemb = pool(self.xlm, batch['input_ids'], batch['pivot_attn_masks'], emb_type)
        return self.estimator(sentemb)

def preprocess_data(triplets_score_dict, tokenizer, max_length, batch_size, shuffle=True, sampler=True, mode='train'):
    if mode == 'train':
        ds = Dataset.from_dict({"pivot": triplets_score_dict['pivot'], 'score': triplets_score_dict['score']})
    else:
        ds = Dataset.from_dict({"pivot": triplets_score_dict['pivot']})
...
def main(lang_dir, gradient_accumulation_steps, batch_size, emb_type, num_epoch, eval_step, \
    ...
    exp_config.hidden_size_FNN=[512, 256]
            ...
            with tqdm(total=len(train_dataloader)) as pbar:
                for step, train_batch in enumerate(train_dataloader):
                    ...
                    if (((step+epoch) % (eval_step * gradient_accumulation_steps) == 0) or (step == len(train_dataloader) - 1)) and rank == 0:
                        ...
                        mt_scores_dict = baselines_cl_eval(mt_outs_dict, refs, emb_type, model, 800, tokenizer)
                        ...
                        if step_seg_cor > cur_best_cor:
                            ...
                            torch.save(model.module, f'{save_dir_name}/best_da_{epoch}_{step}.ckpt')
